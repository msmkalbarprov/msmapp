	...
	public function add(){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect(base_url('proyek/tambah-proyek'),'refresh');
			}
			else{
				$data = array(
				...
				if($result){
					...
					redirect(base_url('proyek/tambah-proyek'));
				}else{
            ...
            $this->load->library('upload', $config);
	            if ( ! $this->upload->do_upload('pic_file')){
	                ...
	                $this->session->set_flashdata('success','Data Rincian Proyek berhasil ditambahkan');
	               	redirect(base_url('proyek/edit-proyek/'.$this->input->post('id_proyek', TRUE)));
	...
	public function edit($id = 0){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
					...
					$this->session->set_flashdata('errors', $data['errors']);
					redirect(base_url('proyek/edit-proyek/'.$id),'refresh');
			}
			else{
				$data = array(
...
public function editRincian($id = 0){
			...
			if ($this->input->post('jnspagu', TRUE)==2 || $this->input->post('jnspagu', TRUE)==3){
				...
				if(!empty($_FILES['pic_file']['name']))
				{
					...
					if($result['status'] == 1){
						...
						redirect(base_url('proyek/edit-proyek-rincian/'.$id), 'refresh');
					}
				}
				...
				if($result){
					$this->session->set_flashdata('success', 'pra redist has been changed Successfully!');
					redirect(base_url('proyek/edit-proyek/'.$id_proyek), 'refresh');
				}
		}
		else{
	...
	public function deleterincian($id = 0)
	{
		$this->rbac->check_operation_access_new('delete');
			...
			$this->session->set_flashdata('success', 'Data berhasil dihapus!');
			redirect(base_url('proyek/edit-proyek/'.$id_proyek));
