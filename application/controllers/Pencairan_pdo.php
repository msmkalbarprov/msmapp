...
class Pencairan_pdo extends MY_Controller {
	...
	public function index(){
		...
		$this->load->view('user/pencairan_pdo/list');
	...
	public function gaji(){
		$data['title'] = 'PDO';
		$this->load->view('admin/includes/_header', $data);
		$this->load->view('user/pencairan_pdo/list_gaji');
		$this->load->view('admin/includes/_footer');
	}
	...
	public function operasional(){
		...
		$this->load->view('admin/includes/_header', $data);
		$this->load->view('user/pencairan_pdo/list_operasional');
		$this->load->view('admin/includes/_footer');
	}


	public function datatable_json(){				   					   
		$records['data'] = $this->pdo_model->get_all_pdo_approve();
		$data = array();
		$i=0;
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($result){
					...
					redirect(base_url('pencairan_pdo/operasional'));
				}else{
					$this->session->set_flashdata('errors', 'PQ Proyek gagal ditambahkan!');
					redirect(base_url('pencairan_pdo/add_operasional'));
				}
			}
		}
...
public function datatable_json_pdo_operasional_edit($id='',$kodepdo=''){				
	...
	public function add_operasional(){
     ...
     //        if($result){
				...
				// 	redirect(base_url('pencairan_pdo/operasional'),'refresh');
				// }else{
				// 	echo json_encode(array(
				// 			"statusCode"=>201
				// 		));
			...
			$this->load->view('user/pencairan_pdo/add_operasional', $data);
			$this->load->view('admin/includes/_footer');
		}
...
public function cair_pdo($id_pdo='',$jns=0)
{		
	$this->rbac->check_operation_access('');
	if($this->input->post('submit')){
			$this->form_validation->set_rules('kd_pdo', 'Kode PDO', 'trim|required');
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$kdpdo 						= $this->security->xss_clean($this->input->post('kd_pdo'));
				$tgl_cair 					= $this->security->xss_clean($this->input->post('tgl_cair'));
				$no_cair 					= $this->security->xss_clean($this->input->post('no_cair'));
				$status 					= 1;
				

				$result = $this->pdo_model->cair_pdo($kdpdo, $status, $no_cair, $tgl_cair);
				if($result){
					$this->activity_model->add_log(1);
			...
			$data['title'] 				= 'Edit PDO';
			$this->load->view('admin/includes/_header');
			$this->load->view('user/pencairan_pdo/cair_proyek', $data);
			$this->load->view('admin/includes/_footer');
		}
...
public function batalcair_pdo($id_pdo='',$jns=0)
{		
	$this->rbac->check_operation_access('');
	if($this->input->post('submit')){
			$this->form_validation->set_rules('kd_pdo', 'Kode PDO', 'trim|required');
			...
			$data['title'] 				= 'Edit PDO';
			$this->load->view('admin/includes/_header');
			$this->load->view('user/pencairan_pdo/batalcair_proyek', $data);
			$this->load->view('admin/includes/_footer');
		}
...
{		
		...
		if($this->input->post('type')==1){
			...
			$data['title'] 				= 'Edit PDO';
			$this->load->view('admin/includes/_header');
			$this->load->view('user/pencairan_pdo/setuju_gaji', $data);
			$this->load->view('admin/includes/_footer');
		}
...
{		
		...
		if($this->input->post('type')==1){
			...
			$data['title'] 				= 'Edit PDO';
			$this->load->view('admin/includes/_header');
			$this->load->view('user/pencairan_pdo/setuju_operasional', $data);
			$this->load->view('admin/includes/_footer');
		}
...
public function delete_pdo_project($id = 0)
	...
	public function delete_pdo_gaji($id = 0)
	{
		$this->rbac->check_operation_access('');
			...
			if ($status==1){
				$this->session->set_flashdata('errors', 'PDO Sudah dicairkan!');
				redirect(base_url('pencairan_pdo/gaji'));				
			}else{
				$this->db->delete('ci_pdo', array('id_pdo' => $id));	
				$this->activity_model->add_log(3);
				$this->session->set_flashdata('success', 'Data berhasil dihapus!');
...
public function delete_pdo_project_temp($id = 0,$kodepdo='',$jns='')
	{
		...
		$this->session->set_flashdata('success', 'Data berhasil dihapus!');
		if ($jns=='1'){
			redirect(base_url('pencairan_pdo/edit_pdo_project/'.$kodepdo));
		}else{
			redirect(base_url('pencairan_pdo/edit_pdo_operasional/'.$kodepdo));	
		}
...
public function delete_pdo_operasional($id = 0)
	...
	public function keranjang_barang(){
		// $this->load->view('admin/includes/_header', $data);
		$this->load->view('user/pencairan_pdo/keranjang');
		// $this->load->view('admin/includes/_footer');
	}

 // function keranjang_barang(){
	// 	$this->rbac->check_operation_access('');
	// 	$this->load->view('user/pencairan_pdo/add');
	// }
	...
	{	
		...
		switch ($jenis)
        {
            case 0;
			    ...
			    $this->pdf->load_view('user/pencairan_pdo/cetak_pdo', $data);
                break;
            case 1;
                echo "<title>Cetak PDO</title>";
                echo $this->load->view('user/pencairan_pdo/cetak_pdo', $data);
               break;
        }
	...
	{	
		...
		switch ($jenis)
        {
            case 0;
			    ...
			    $this->pdf->setPaper('Legal', 'landscape');
			    $this->pdf->filename = "laporan_pdo.pdf";
			    $this->pdf->load_view('user/pencairan_pdo/cetak_pdo_gaji', $data);
                break;
            case 1;
                echo "<title>Cetak PDO</title>";
                echo $this->load->view('user/pencairan_pdo/cetak_pdo_gaji', $data);
               break;
        }
	...
	{	
		...
		switch ($jenis)
        {
            case 0;
			    ...
			    $this->pdf->setPaper('Legal', 'landscape');
			    $this->pdf->filename = "laporan_pdo.pdf";
			    $this->pdf->load_view('user/pencairan_pdo/cetak_pdo_operasional', $data);
                break;
            case 1;
                echo "<title>Cetak PDO</title>";
                echo $this->load->view('user/pencairan_pdo/cetak_pdo_operasional', $data);
               break;
        }
