...
class Penerimaan_pdo extends MY_Controller {
	...
	public function datatable_json(){				   					   
		...
		{  
				...
				if ($row['status_terima']=='1'){
					$tombol = '<div class="text-center"><a title="Batal" class="update btn btn-sm btn-success" href="'.base_url('penerimaan_pdo/batalterima_pdo/'.str_replace("/","",$row['id_pdo'])).'"> <i class="fa fa-check"></i></a></div';
				}else{
					$tombol = '<div class="text-center"><a title="Cair" class="update btn btn-sm btn-info" href="'.base_url('penerimaan_pdo/terima_pdo/'.str_replace("/","",$row['id_pdo'])).'"> <i class="fa fa-list"></i></a></div';
				}
			...
			$data[]= array(
				...
				'<font size="2px">'.$row['tgl_terima'].'</font>',
				'<font size="2px">'.$row['no_terima'].'</font>',
...
public function datatable_json_pdo_operasional_edit($id='',$kodepdo=''){				
	...
	public function add_operasional(){
     ...
     //        if($result){
				...
				// 	redirect(base_url('penerimaan_pdo/operasional'),'refresh');
				// }else{
				// 	echo json_encode(array(
				// 			"statusCode"=>201
				// 		));
			...
			$this->load->view('user/penerimaan_pdo/add_operasional', $data);
			$this->load->view('admin/includes/_footer');
		}
...
public function terima_pdo($id_pdo='',$jns=0)
{		
	$this->rbac->check_operation_access('');
	if($this->input->post('submit')){
			$this->form_validation->set_rules('kd_pdo', 'Kode PDO', 'trim|required');
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$kdpdo 						= $this->security->xss_clean($this->input->post('kd_pdo'));
				$tgl_terima 				= $this->security->xss_clean($this->input->post('tgl_terima'));
				$status 					= 1;
				

				$result = $this->pdo_model->terima_pdo($kdpdo, $status,$tgl_terima);
				if($result){
					$this->activity_model->add_log(1);
					$this->session->set_flashdata('success', 'PDO berhasil dicairkan!');
			...
			$this->load->view('user/penerimaan_pdo/terima_proyek', $data);
			$this->load->view('admin/includes/_footer');
		}
...
public function batalterima_pdo($id_pdo='',$jns=0)
{		
	$this->rbac->check_operation_access('');
	if($this->input->post('submit')){
			$this->form_validation->set_rules('kd_pdo', 'Kode PDO', 'trim|required');
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$result = $this->pdo_model->terima_pdo($kdpdo, $status, $no_cair, $tgl_terima);
				if($result){
					$this->activity_model->add_log(1);
					$this->session->set_flashdata('success', 'PDO berhasil dibatalcairkan!');
					redirect(base_url('penerimaan_pdo'));
			...
			$data['title'] 				= 'Edit PDO';
			$this->load->view('admin/includes/_header');
			$this->load->view('user/penerimaan_pdo/batalterima_proyek', $data);
			$this->load->view('admin/includes/_footer');
		}
...
{		
		...
		if($this->input->post('type')==1){
			...
			$data['title'] 				= 'Edit PDO';
			$this->load->view('admin/includes/_header');
			$this->load->view('user/penerimaan_pdo/setuju_gaji', $data);
			$this->load->view('admin/includes/_footer');
		}
...
{		
		...
		if($this->input->post('type')==1){
			...
			$data['title'] 				= 'Edit PDO';
			$this->load->view('admin/includes/_header');
			$this->load->view('user/penerimaan_pdo/setuju_operasional', $data);
			$this->load->view('admin/includes/_footer');
		}
...
public function delete_pdo_project($id = 0)
	...
	public function delete_pdo_gaji($id = 0)
	{
		$this->rbac->check_operation_access('');
			...
			if ($status==1){
				$this->session->set_flashdata('errors', 'PDO Sudah dicairkan!');
				redirect(base_url('penerimaan_pdo/gaji'));				
			}else{
				$this->db->delete('ci_pdo', array('id_pdo' => $id));	
				$this->activity_model->add_log(3);
				$this->session->set_flashdata('success', 'Data berhasil dihapus!');
...
public function delete_pdo_project_temp($id = 0,$kodepdo='',$jns='')
	{
		...
		$this->session->set_flashdata('success', 'Data berhasil dihapus!');
		if ($jns=='1'){
			redirect(base_url('penerimaan_pdo/edit_pdo_project/'.$kodepdo));
		}else{
			redirect(base_url('penerimaan_pdo/edit_pdo_operasional/'.$kodepdo));	
		}
...
public function delete_pdo_operasional($id = 0)
	...
	public function keranjang_barang(){
		// $this->load->view('admin/includes/_header', $data);
		$this->load->view('user/penerimaan_pdo/keranjang');
		// $this->load->view('admin/includes/_footer');
	}

 // function keranjang_barang(){
	// 	$this->rbac->check_operation_access('');
	// 	$this->load->view('user/penerimaan_pdo/add');
	// }
	...
	{	
		...
		switch ($jenis)
        {
            case 0;
			    ...
			    $this->pdf->load_view('user/penerimaan_pdo/cetak_pdo', $data);
                break;
            case 1;
                echo "<title>Cetak PDO</title>";
                echo $this->load->view('user/penerimaan_pdo/cetak_pdo', $data);
               break;
        }
	...
	{	
		...
		switch ($jenis)
        {
            case 0;
			    ...
			    $this->pdf->setPaper('Legal', 'landscape');
			    $this->pdf->filename = "laporan_pdo.pdf";
			    $this->pdf->load_view('user/penerimaan_pdo/cetak_pdo_gaji', $data);
                break;
            case 1;
                echo "<title>Cetak PDO</title>";
                echo $this->load->view('user/penerimaan_pdo/cetak_pdo_gaji', $data);
               break;
        }
	...
	{	
		...
		switch ($jenis)
        {
            case 0;
			    ...
			    $this->pdf->setPaper('Legal', 'landscape');
			    $this->pdf->filename = "laporan_pdo.pdf";
			    $this->pdf->load_view('user/penerimaan_pdo/cetak_pdo_operasional', $data);
                break;
            case 1;
                echo "<title>Cetak PDO</title>";
                echo $this->load->view('user/penerimaan_pdo/cetak_pdo_operasional', $data);
               break;
        }
