...
class Bank extends MY_Controller
{
	...
	function index($subarea=''){
		...
		$this->load->view('user/bank/index', $data);
		$this->load->view('admin/includes/_footer');
	}
	//--------------------------------------------------		

public function datatable_json(){				   					   
		$records['data'] = $this->bank->get_all();
		$data = array();
		...
		{  
			...
			$data[]= array(
				...
				$row['pemilik'],
				$row['nm_bank'],
				'<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('bank/edit/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a>
				<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("bank/delete/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>'
			);
		}
		$records['data']=$data;
	...
	function add(){
		...
		if($this->input->post('submit')){
				...
				if ($this->form_validation->run() == FALSE) {
					...
					redirect(base_url('bank/add'),'refresh');
				}
				else{
					$data = array(
	...
	function edit($id=""){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->session->set_flashdata('errors', $data['errors']);
				redirect(base_url('bank/edit/'.$id),'refresh');
			}
			else{
				$data = array(
					'no_rekening' 	=> $this->input->post('no_rekening'),
				...
				$data = $this->security->xss_clean($data);
				$result = $this->bank->edit_bank($data, $id);

				if($result){
					// Activity Log 
					$this->activity_model->add_log(5);

					$this->session->set_flashdata('success', 'bank berhasil diupdate!');
					redirect(base_url('bank/index'));
				}
			}
		}
