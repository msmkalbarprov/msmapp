        ...
        <div class="card-header">
          <div class="d-inline-block">
            <h3 class="card-title"> <i class="fa fa-print"></i>
				Cetak Rincian Realisasi Proyek Cair SPJ</h3>
           </div>
			...
			<div class="row" style="margin:10px">
				...
				<div class="row" style="margin:10px">
					<div class="col-md-12">
						...
						<div class="form-group">
							<label class="col-sm-1 control-label input-sm">Akun</label>

							<div class="col-sm-6">
										<select name="cbacc" id="cbacc" class="form-control input-sm" style="width:100%;">
										</select>
...
$('#thn_ang').select2({
	...
	$('#cbarea').on('change', function(){ 
		...
		var ctahun		= $('#thn_ang').val();
			...
			$.ajax({
				  type: 'POST',
				  url: "<?php echo base_url(); ?>laporan-Realisasi/list-acc",
        ...
        $(document).on("click", "#search", function() {
			...
			if (ctahun == '') {
        		...
        		$("#treeview-mdbootstrap").html('<center><img src="<?php echo base_url('assets/img/loading.gif'); ?>" alt="Loading" height="42" width="42"></center>');
			  	$.ajax({
					  url: '<?php echo base_url('laporan-rinci-realisasi-proyek-spj-prev'); ?>',
					  type: 'POST',
					   data:{'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',ctahun,carea,cacc,ctipe},
					  success: function(data){
        ...
        $(document).on("click", ".tombol-print", function() {
        	...
        	if (ctahun == '') {
        		...
        		swal.fire({
			  	...
			  	if (id == 'print-pdf') {
			  		window.open('<?php echo base_url('laporan-rinci-realisasi-proyek-spj-pdf'); ?>/'+ctahun+'/'+carea+'/'+cacc+'/pdf', '_blank');	
			  	}else{
					window.open('<?php echo base_url('laporan-rinci-realisasi-proyek-spj-excel'); ?>/'+ctahun+'/'+carea+'/'+cacc+'/excel', '_blank');
			  	}
        	}
