      ...
      <div class="card card-default color-palette-bo">
        <div class="card-header">
          <div class="d-inline-block">
              <h3 class="card-title">
              Tambah Pengembalian </h3>
          </div>
          <div class="d-inline-block float-right">
            <a href="<?= base_url('pengembalian/index'); ?>" class="btn btn-success"><i class="fa fa-list"></i> List Pengembalian</a>
          </div>
        </div>
        <div class="card-body">   
           <!-- For Messages -->
  ...
  $(document).ready(function(){
    ...
    $('#area').change(function(){ 
        $('#saldo').val('0,00');
                ...
                $.ajax({
                    url : "<?php echo site_url('pengembalian/get_pegawai_by_area');?>",
                    method : "POST",
                    data : {
            ...
            $('#kd_pegawai').change(function(){ 
                    ...
                    $.ajax({
                        url : "<?php echo site_url('pengembalian/get_kas');?>",
                        method : "POST",
                        data : {
                        '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
                        id: area,kd_pegawai:kd_pegawai},
                        async : true,
                        dataType : 'json',
                        success: function(data){
                            $.each(data, function(key, value) {
