    ...
    <section class="content">
      <div class="card card-default">
        <div class="card-header">
          <div class="d-inline-block">
           ...
           <div class="d-inline-block float-right">
            <a href="<?= base_url('spj_tunai'); ?>" class="btn btn-primary btn-sm"><i class="fa fa-reply"></i>  kembali</a>
          </div>
        </div>
        <div class="card-body">
        ...
        <?php $this->load->view('admin/includes/_messages.php') ?>
         <?php echo form_open(base_url('spj_tunai/add_spj'), 'class="form-horizontal"' )?> 
         <div class="row">
          <div class="col-md-4">
...
<script>
  $(document).ready(function(){
    ...
    "ajax": 
    {
                "url": "<?= base_url('spj_tunai/view'.'/')?>"+ nospj+'/'+kd_pegawai, // URL file untuk proses select datanya
                "type": "POST",
                "data" : {
...
function get_pegawai(area) {
    $.ajax({
                    url : "<?php echo site_url('spj_tunai/get_pegawai_by_area');?>",
                    method : "POST",
                    data : {
                      '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
                      id: area},
...
$('#na_datatable').on('click', 'tbody .del_btn', function () {
    ...
    // proses hapus
    $.ajax({
        url: "<?php echo base_url("spj_tunai/delete_spj_temp2/");?>",
        type: "POST",
        data: {
          '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
          type:1,
...
function load_rincian_temp(nospj) {
        ...
        table.ajax.url("<?=base_url('spj_tunai/view'.'/')?>"+ nospj+'/'+kd_pegawai);
        table.ajax.reload();
}

function get_akun(jns_spj,kd_proyek){
    $.ajax({
        url : "<?php echo site_url('spj_tunai/get_item_spj');?>",
        method : "POST",
        data : {
          '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
...
function get_nomor_urut(kd_pegawai){
        $.ajax({
        url : "<?php echo site_url('spj_tunai/get_nomor');?>",
        method : "POST",
        data : {
          '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
...
$('#formtest').submit(function(e){
            ...
            $.ajax({
                url:'<?php echo base_url();?>index.php/spj_tunai/simpan_spj_file',
...
//       $.ajax({
//         url: "<?php echo base_url("spj_tunai/add/");?>",
//         type: "POST",
//         data: {
//             '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
//             type:1,
