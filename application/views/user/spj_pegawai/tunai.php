      ...
      <div class="card-header">
          <div class="d-inline-block">
            <h3 class="card-title"> <i class="fa fa-list"></i>
             SPJ Pegawai Tunai</h3>
           </div>
           <div class="d-inline-block float-right">
            <a href="<?= base_url('spj_tunai/add'); ?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i> Tambah</a> 
...
<script>
  ...
  var table = $('#na_datatable').DataTable( {
    "processing": true,
    "serverSide": false,
    "ajax": "<?=base_url('spj_tunai/datatable_json')?>",
    "order": [[0,'asc']],
    "columnDefs": [
  ...
  function get_pegawai() {
    $.ajax({
          url : "<?php echo site_url('spj_tunai/get_pegawai_by_area_cetak');?>",
          method : "POST",
          data : {
...
function get_area_by_user(kd_pegawai) {
    $.ajax({
          url : "<?php echo site_url('spj_tunai/get_area_by_user');?>",
          method : "POST",
          data : {
            '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
            id:kd_pegawai},
...
$('#but_cetak2').on('click', function() {
  ...
  var area              = $('#area').val();
      ...
      var url = '<?= base_url() ?>'+'spj_pegawai/cetak_spj_karyawan'+kd_pegawai+'/'+area+'/'+bulan+'/'+tahun+'/0/Laporan SPJ';   
    window.open(url, '_blank');
