    ...
    <section class="content">
      <div class="card card-default">
        ...
        <?php $this->load->view('admin/includes/_messages.php') ?>
         <?php echo form_open_multipart('penerimaan_pdo/batalterima_pdo/'.$data_pdo["id_pdo"].'/1');?>
...
function get_nomor_urut(){
    var no_terima   = "<?= $data_pdo["no_terima"]; ?>";
    var tgl_terima  = "<?= $data_pdo["tgl_terima"]; ?>";
        $.ajax({
        url : "<?php echo site_url('penerimaan_pdo/get_nomor_bud');?>",
        method : "POST",
        data : {
        ...
        success: function(data){
            $.each(data, function(key, value) {
                ...
                if (no_terima=='' || no_terima== null){
                  $('[name="no_terima"]').val(value.nomor).trigger('change');  
                }else{
                  $('[name="no_terima"]').val(no_terima).trigger('change');
                  $('[name="tgl_terima"]').val(tgl_terima).trigger('change');
