...
class pembayarantahunlalu_model extends CI_Model{
...
public function get_all(){
		...
		if($this->session->userdata('is_supper') || $this->session->userdata('admin_role')=='Direktur Utama' || $this->session->userdata('admin_role')=='Divisi Administrasi Proyek' || $this->session->userdata('admin_role')=='Admin' ){
			$this->db->select('ci_pembayaran_tahun_lalu.*,(select ifnull(sum(nilai),0) from ci_pembayaran_tahun_lalu_potongan where ci_pembayaran_tahun_lalu.no_bukti=ci_pembayaran_tahun_lalu_potongan.no_kas and ci_pembayaran_tahun_lalu_potongan.no_kas=ci_pembayaran_tahun_lalu.no_bukti) as potongan');
			$this->db->from('ci_pembayaran_tahun_lalu');
			$this->db->where('jenis', 'tahunlalu');
        ...
        public function simpan($data){
            $this->db->insert('ci_pembayaran_tahun_lalu', $data);
            return true;
        }
    ...
    function get_potongan_by_id($id)
	{
		$this->db->from('ci_pembayaran_tahun_lalu');
    ...
    public function simpan_potongan($data){
        $this->db->insert('ci_pembayaran_tahun_lalu_potongan', $data);
        return true;
    }

    public function get_potongan_pembayarantahunlalu_by_id($id){
        $this->db->from("ci_pembayaran_tahun_lalu_potongan");
...
function get_kas_rekening($id)
    ...
    function get_pembayarantahunlalu_by_id($id)
	{
		$this->db->from('ci_pembayaran_tahun_lalu');
		$this->db->where('id',$id);
		$query=$this->db->get();
...
function get_plain_by_id($id)
	...
	//---------------------------------------------------
	// Edit Admin Record
public function edit_pembayarantahunlalu($data, $id){
	$this->db->where('id', $id);
	$this->db->update('ci_pembayaran_tahun_lalu', $data);
	return true;
}
...
{		
	$this->db->where('id',$id);
	$this->db->delete('ci_pembayaran_tahun_lalu');
} 
