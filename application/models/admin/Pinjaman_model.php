...
class Pinjaman_model extends CI_Model{
	...
	public function get_all(){
		...
		function get_pegawai_by_area($area)
		{
			...
			$this->db->where('LEFT(kd_pegawai,2)', 'PG');
			$query=$this->db->get();
			return $query;
		}
	...
	function get_pinjaman_potongan_by_id($id,$pegawai)
	{
		$this->db->from('ci_pinjaman');
		$this->db->where('no_bukti',$id);
		$this->db->where('kd_pegawai',$pegawai);
		$query=$this->db->get();
		return $query->row_array();
	}

	function get_pinjaman_by_id($id,$pegawai)
	{
		$this->db->from('ci_pinjaman');
		$this->db->where('id',$id);
		$this->db->where('kd_pegawai',$pegawai);
...
public function get_potongan_pinjaman_by_id($id){
	$this->db->from("ci_pinjaman_potongan");
...
public function simpan_potongan_pinjaman($data){
	// $this->db->insert('ci_pelimpahan_potongan', $data);
	...
	$this->db->insert('ci_pinjaman_potongan', $insert_data);
...
public function edit_pinjaman($data, $id, $pegawai){
	$this->db->where('id', $id);
	$this->db->where('kd_pegawai', $pegawai);
	$this->db->update('ci_pinjaman', $data);
	return true;
}
...
function get_plain_by_id($id)
	...
	{
		if ($divisi=='3' && ($area=='DD00' || $area=='DD02' || $area=='DD04' || $area=='DD05' || $area=='DD08' || $area=='DD15' || $area=='DD17' || $area=='DD19' || $area=='BB02' || $area=='EE01' || $area=='FF01' || $area=='FF02')){
